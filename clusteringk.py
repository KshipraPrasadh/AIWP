...
num_clusters=5
plt.figure()
plt.scatter(X[:,0],X[:,1],marker='o',facecolors='blue',edgecolors='black',s=100)
...
plt.scatter(cluster_centers[:,0],cluster_centers[:,1],marker='o',s=210,linewidths=4,color='yellow',zorder=12,facecolors='yellow')
x_min,x_max=X[:,0].min()-1,X[:,0].max()+1
y_min,y_max=X[:,1].min()-1,X[:,1].max()+1
